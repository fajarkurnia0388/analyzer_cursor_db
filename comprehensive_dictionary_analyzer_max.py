...
Versi MAX - Menampilkan hasil lengkap tanpa batasan
Author: AI Assistant
"""
...
class ComprehensiveDictionaryAnalyzerMax:
    def __init__(
        self, db_path, output_dir="dictionary_analysis_max", max_display_examples=None
    ):
        self.db_path = db_path
        self.output_dir = Path(output_dir)
        self.max_display_examples = max_display_examples  # None = unlimited
        self.stats = {
            "total_words": 0,
            "total_phrases": 0,
            "unique_words": 0,
    ...
    def _save_words(self):
        ...
        for letter in string.ascii_uppercase:
            ...
            # Simpan sebagai TXT dengan tampilan lengkap
            txt_file = letter_dir / f"{letter}_words.txt"
            with open(txt_file, "w", encoding="utf-8") as f:
                f.write(f"DICTIONARY WORDS - LETTER {letter} (FULL RESULTS)\n")
                f.write("=" * 60 + "\n\n")
                ...
                for word, entries in sorted(self.word_dictionary[letter].items()):
                    ...
                    # Tampilkan semua contoh konteks (tidak dibatasi)
                    max_examples = (
                        self.max_display_examples
                        if self.max_display_examples
                        else len(entries)
                    )
                    ...
                    if len(entries) > max_examples:
                        remaining = len(entries) - max_examples
                        f.write(
                            f"      ... and {remaining:,} more occurrences (truncated for display)\n"
                        )
            ...
            with open(csv_file, "w", newline="", encoding="utf-8") as f:
                writer = csv.writer(f)
                writer.writerow(
                    ["Word", "Frequency", "Table", "Column", "Context", "Timestamp"]
                )
    ...
    def _save_phrases(self):
        ...
        for letter in string.ascii_uppercase:
            ...
            with open(txt_file, "w", encoding="utf-8") as f:
                f.write(f"DICTIONARY PHRASES - LETTER {letter} (FULL RESULTS)\n")
                f.write("=" * 60 + "\n\n")
    ...
    def generate_statistics(self):
        ...
        general_stats = {
            "analysis_info": {
                ...
                "max_display_examples": self.max_display_examples,
            },
            "word_statistics": {
                "total_words": self.stats["total_words"],
                "unique_words": len(self.word_frequencies),
                "most_common_words": dict(
                    self.word_frequencies.most_common(100)
                ),  # Lebih banyak
            },
            "phrase_statistics": {
                "total_phrases": self.stats["total_phrases"],
                "unique_phrases": len(self.phrase_frequencies),
                "most_common_phrases": dict(
                    self.phrase_frequencies.most_common(100)
                ),  # Lebih banyak
            },
            "letter_distribution": {
                "words_by_letter": {
        ...
        with open(stats_dir / "analysis_report.txt", "w", encoding="utf-8") as f:
            f.write("COMPREHENSIVE DICTIONARY ANALYSIS REPORT (MAX VERSION)\n")
            f.write("=" * 70 + "\n\n")
            ...
            f.write(
                f"Display Limit: {'Unlimited' if self.max_display_examples is None else str(self.max_display_examples)}\n\n"
            )
            ...
            f.write("-" * 15 + "\n")
            for i, (word, freq) in enumerate(self.word_frequencies.most_common(50), 1):
                f.write(f"{i:2d}. {word}: {freq:,}\n")

            f.write("\nTOP 50 PHRASES:\n")
            f.write("-" * 17 + "\n")
            for i, (phrase, freq) in enumerate(
                self.phrase_frequencies.most_common(50), 1
            ):
                f.write(f"{i:2d}. {phrase}: {freq:,}\n")
...
<head>
    ...
    <title>Dictionary Analysis Report (MAX) - {Path(self.db_path).name}</title>
    <style>
        ...
        .max-badge {{ background: #28a745; color: white; padding: 2px 6px; border-radius: 3px; font-size: 12px; }}
    </style>
</head>
<body>
    <div class="container">
        <h1>📚 Comprehensive Dictionary Analysis Report <span class="max-badge">MAX VERSION</span></h1>
        <p><strong>Database:</strong> {Path(self.db_path).name}</p>
        <p><strong>Analysis Date:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
        <p><strong>Display Mode:</strong> {'Unlimited Results' if self.max_display_examples is None else f'Max {self.max_display_examples} examples per item'}</p>
...
│       └── 📄 {letter}_phrases.csv"""
        ...
        <ul>
            ...
            <li><strong>MAX Version:</strong> This version shows complete results without truncation</li>
        </ul>
    </div>
</body>
</html>
"""
    ...
    def run_comprehensive_analysis(self):
        ...
        print(
            "🎯 [COMPREHENSIVE DICTIONARY ANALYZER MAX] Analisis Dictionary Lengkap - Versi MAX"
        )
        print("=" * 90)
        ...
        print("✨ Menampilkan hasil LENGKAP tanpa batasan")
        print("=" * 90)
        ...
        try:
            ...
            print("\n🎉 [COMPLETED] Analisis dictionary MAX selesai!")
...
def main():
    print(
        "🚀 [COMPREHENSIVE DICTIONARY ANALYZER MAX] Script Analisis Dictionary Komprehensif - Versi MAX"
    )
    print("=" * 100)

    # Parse command line arguments
    max_examples = None  # Default: unlimited
    db_path = None

    i = 1
    while i < len(sys.argv):
        arg = sys.argv[i]
        if arg == "--max-examples" and i + 1 < len(sys.argv):
            try:
                max_examples = int(sys.argv[i + 1])
                i += 2
            except ValueError:
                print(f"❌ [ERROR] Invalid --max-examples value: {sys.argv[i + 1]}")
                return
    ...
    if not db_path:
        ...
        print(
            "📍 [USAGE] python comprehensive_dictionary_analyzer_max.py [path_to_state.vscdb] [--max-examples N]"
        )
        print("📍 [EXAMPLES]:")
        print(
            "   python comprehensive_dictionary_analyzer_max.py                    # Unlimited results"
        )
        print(
            "   python comprehensive_dictionary_analyzer_max.py --max-examples 50  # Max 50 examples per item"
        )
        return

    print(f"🗃️  [DATABASE] Menggunakan file: {db_path}")
    print(
        f"📊 [DISPLAY] Mode tampilan: {'Unlimited' if max_examples is None else f'Max {max_examples} examples per item'}"
    )
    ...
    if file_size > 100 * 1024 * 1024:  # > 100MB
        ...
        print(
            "Analisis MAX akan memproses semua teks dan membuat file hasil yang sangat detail."
        )
        confirm = input("Lanjutkan analisis MAX? (y/n): ").lower()
        if confirm not in ["y", "yes"]:
            print("❌ [CANCELLED] Analisis dibatalkan")
            return

    # Inisialisasi analyzer
    analyzer = ComprehensiveDictionaryAnalyzerMax(
        db_path, max_display_examples=max_examples
    )
